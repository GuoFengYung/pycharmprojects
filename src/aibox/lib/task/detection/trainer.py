...
class Trainer:
    ...
    def __init__(self, config: Config, logger: Logger, augmenter: Augmenter,
        ...
        val_evaluator = Evaluator(self.val_dataset, batch_size=device_count, num_workers=config.num_workers, quality=config.eval_quality)
        test_evaluator = Evaluator(self.test_dataset, batch_size=device_count, num_workers=config.num_workers, quality=config.eval_quality) if num_test_data > 0 else None
