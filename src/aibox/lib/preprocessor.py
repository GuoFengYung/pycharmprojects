...
class Preprocessor:
    ...
    def process(self, image: Union[PIL.Image.Image, Tensor], is_train_or_eval: bool) -> Tuple[Tensor, Dict[str, Any]]:
        if isinstance(image, PIL.Image.Image):
            image = to_tensor(image)

        image_width, image_height = image.shape[2], image.shape[1]

        scale_for_width = 1 if self.image_resized_width == -1 else self.image_resized_width / image_width
        scale_for_height = 1 if self.image_resized_height == -1 else self.image_resized_height / image_height
        ...
        if self.image_min_side == -1:
            ...
            scale_for_shorter_side = self.image_min_side / min(image_width * scale_for_width,
                                                               image_height * scale_for_height)
        ...
        process_dict = {
            ...
            self.PROCESS_KEY_ORIGIN_WIDTH: image_width,
            self.PROCESS_KEY_ORIGIN_HEIGHT: image_height,
